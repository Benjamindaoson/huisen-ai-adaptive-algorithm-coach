...
export function LessonPage({ lesson, transferProblem, onSignal, onOpenProblem }: Props) {
  ...
  useEffect(() => {
    setStage(0); setFrame(0); setSelected(null); setAnswer(''); setCompletionError(false);
  ...
  function startTransfer() {
    ...
    onSignal({ kind: 'lesson-transfer-started', problemId: transferProblem.id, data: { lessonId: lesson.id, stage: 'transfer' } });
    onOpenProblem(transferProblem.id);
  }

  return <div className="lesson-page">
    <header className="lesson-header"><div><span className="section-kicker">FOUNDATION · {String(lesson.order).padStart(2, '0')}</span><h1>{lesson.title}</h1><p>{lesson.professionalName} · 约 {lesson.minutes} 分钟</p></div><div className="lesson-goal"><span>这节学会</span><strong>{lesson.objective}</strong></div></header>
