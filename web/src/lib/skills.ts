export const OD_SKILLS = [
  ...
  { id: 'sorting', title: '排序', description: '排序、比较器、Top K 与有序结构。', keywords: ['排序', '有序', 'topk', 'top k', '优先级', '排名', '名次', '比较器', '前3名'] },
  ...
  { id: 'greedy', title: '贪心', description: '识别局部最优选择及其正确性条件。', keywords: ['贪心', 'greedy'] },
...
type SearchableProblem = {
  ...
  classification?: { source: 'verified' | 'candidate' | 'inferred'; confidence: number };
};
...
export function inferProblemSkills(problem: SearchableProblem): SkillId[] {
  ...
  if (persisted?.length && (!problem.classification || problem.classification.source === 'verified')) return [...new Set(persisted)];
  ...
  return persisted?.length ? [...new Set(persisted)] : ['simulation'];
}
