...
function milestoneKey(event: LearningEvent): string | null {
  ...
  if (event.kind === 'bridge-diagnostic-step-recorded') return event.data.diagnosticStep ? `diagnostic:${event.data.diagnosticStep}` : null;
  if (event.kind === 'lesson-completed' || event.kind === 'lesson-transfer-passed') return event.data.lessonId ? `${event.kind}:${event.data.lessonId}` : null;
  if (event.kind === 'training-session-started' || event.kind === 'training-session-completed') return event.data.lessonId ? `${event.kind}:${event.data.lessonId}` : null;
  if (event.kind === 'training-stage-completed') return event.data.lessonId && event.data.stage ? `${event.kind}:${event.data.lessonId}:${event.data.stage}` : null;
