...
function accessMetadata(source:SourceAccess):Record<string,unknown> {
  return {
...
export type CourseDoc=SourceAccess & Readonly<{id:string;title:string;text:string;sourcePath:string;tags?:readonly string[]}>;
export type CodeSymbol=SourceAccess & Readonly<{id:string;symbolName:string;symbolType:string;text:string;sourcePath:string;startLine:number;endLine:number}>;
export type ErrorRecipe=SourceAccess & Readonly<{id:string;errorPattern:string;symptom?:string;cause:string;fixSteps:readonly string[];verifyCommand?:string;sourcePath:string}>;
export type FaqRecord=SourceAccess & Readonly<{id:string;question:string;answer:string;sourcePath:string}>;
...
export function retrieveCourse(query:string, docs:readonly CourseDoc[], route:'course'|'learning_path', limit=5):TutorCandidate[] {
  ...
  return scored.map(({doc,score})=>({
    ...
    title:doc.title,
    metadata:{tags:doc.tags??[],...accessMetadata(doc)}
  }));
}
...
export function retrieveCode(query:string, symbols:readonly CodeSymbol[], limit=5):TutorCandidate[] {
  ...
  return symbols.map(symbol=>{
    ...
    metadata:{symbolType:symbol.symbolType,...accessMetadata(symbol)}
  }));
}
...
export function retrieveFaq(query:string, faqs:readonly FaqRecord[], threshold=0.58):TutorCandidate[] {
  ...
  return [{
    id:best.f.id,
    text:best.f.answer,
    sourcePath:best.f.sourcePath,
    sourceType:'faq',
    score:best.score,
    title:best.f.question,
    metadata:accessMetadata(best.f)
  }];
}
