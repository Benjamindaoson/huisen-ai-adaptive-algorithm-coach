...
export class TutorRuntime {
  ...
  run(request:TutorRequest):TutorResponse {
    ...
    this.record(request,traceId,'tool_trace','route',{route:route.route,reason:route.reason,confidence:route.confidence});

    if(route.needsClarification){
      const action: TutorAction='clarify';
      this.record(request,traceId,'learning_event','intervention',{action,reason:route.reason});
    ...
    this.record(request,traceId,'retrieval_trace','retrieval',{
      route:route.route,
    ...
    this.record(request,traceId,'tool_trace','evidence_gate',{action:gate.action,reason:gate.reason,confidence:gate.confidence});

    const citations=citationsFrom(candidates);
    if(gate.action!=='accept'){
      const action: TutorAction='refuse';
      this.record(request,traceId,'learning_event','intervention',{action,reason:gate.reason});
      return {
    ...
    this.record(request,traceId,'learning_event','intervention',{
      action,
  ...
  private record(request:TutorRequest,traceId:string,evidenceType:EvidenceEvent['evidenceType'],phase:string,metadata:Record<string,unknown>):void {
