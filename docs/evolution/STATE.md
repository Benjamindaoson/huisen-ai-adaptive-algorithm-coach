...
## Active evaluation contract — README product-intelligence architecture

Status: closed with KEEP decision in Run 50.

### Hypothesis

Replacing the infrastructure-first Mermaid topology with one branded product-intelligence diagram will make the README communicate the product's adaptive-learning and verifiable-Agent differentiation before readers inspect implementation details.

### Observed baseline

- The current diagram gives seven infrastructure or service boxes similar visual weight and centers `Learning & Mentor Gateway`.
- It names React, PostgreSQL, Redis, MinIO, Judge0, and DeepSeek, but does not make the learner journey, next-best-action decision, Agent evidence loop, or transfer/delayed-review feedback loop visible.
- The primary product promise — easier learning, durable mastery, and real application — is absent from the diagram.
- The Mermaid renderer exposes zoom and navigation controls in GitHub's expanded view, which makes the result feel like an autogenerated topology instead of a product artifact.

### Primary proxy metric

A diagram-content rubric scored from the rendered artifact. A pass requires all five product-intelligence layers to be visible and hierarchically distinct:

1. learner-facing experience;
2. adaptive learning orchestration;
3. Mentor observe/plan/tool/verify/teach/replan-or-stop loop;
4. trusted learning memory and transfer/delayed-review feedback;
5. de-emphasized engineering and safety foundation.

Baseline: 2/5 layers are partially visible (`experience` and `foundation`), neither as a clear layer.

Target: 5/5 layers visible, with the three learner outcomes readable before infrastructure details.

### Guardrails

- Do not claim unimplemented image/voice capability, validated learning outcomes, production readiness, or teacher-adjudicated Mentor quality.
- Keep the architecture consistent with the repository's current React/Gateway/PostgreSQL/Redis/MinIO/Judge0/DeepSeek boundaries.
- Use a pure SVG without `foreignObject`, remote fonts, JavaScript, or external assets so GitHub can render it safely.
- Preserve the README's existing quickstart and factual status boundaries.
- The SVG must have no clipped text or overlapping cards in a full-size render, remain understandable when embedded at README width, and link to the full-resolution asset.

### Evaluation method

- Validate SVG as XML and inspect its text inventory against the five-layer rubric.
- Render the SVG to a raster preview and inspect it visually at full size and a narrow README-equivalent width.
- Run `git diff --check` and scan the README for the old architecture Mermaid block.
- Push the focused documentation change and inspect the public GitHub README rendering.

### Revert threshold

Revert if any required layer is missing, any text overlaps or clips, the asset fails GitHub rendering, infrastructure again dominates the hierarchy, or the diagram introduces a capability claim unsupported by the repository.

## Run 50 — README product-intelligence architecture

Date: 2026-08-17

### Improvement

- Replaced the infrastructure-first Mermaid topology with a self-contained 1600×1060 branded SVG.
- Moved the learner promise to the visual header: easier learning, durable mastery, and real application.
- Made five distinct layers visible: learner experience, adaptive orchestration, Mentor Agent runtime, trusted learning memory, and the engineering/safety foundation.
- Expanded the Mentor box into an evidence loop: observe context → form a hypothesis → select a tool → verify the diagnosis → teach minimally → replan or stop.
- Added the evidence-feedback path from transfer and delayed review into the next-best-action decision.
- De-emphasized vendor infrastructure into a thin bottom foundation while preserving the current React, Gateway, DeepSeek, Judge0, PostgreSQL, Redis, and MinIO boundaries.
- Rewrote the README explanation around the product's evidence loop instead of listing framework responsibilities.

### Evaluation

| Measure | Before | After | Result |
|---|---:|---:|---|
| Product-intelligence layers visible | 2/5 partial | 5/5 distinct | Target met |
| Learner outcomes visible in diagram | 0/3 | 3/3 | Target met |
| Mentor runtime steps visible | 1 generic box | 6-step verified loop | Target met |
| Infrastructure visual priority | Central topology | 92 px foundation strip | De-emphasized |
| Old Mermaid in architecture section | Present | Absent | Removed |
| SVG XML validity | Not applicable | Valid | Passed |
...
- Product source code and runtime behavior were not changed, so the application test suite was not rerun or presented as new product evidence.

### Decision

KEEP. The architecture now leads with the product's adaptive and verifiable learning loop, while the infrastructure remains available as supporting evidence instead of becoming the story.

### Learned

- A product architecture diagram should explain the mechanism that creates user value; a deployment topology should be a separate technical artifact.
- The strongest differentiator is not the presence of a model but the connection between Agent tool evidence, learner memory, transfer validation, and the next training decision.
- At narrow README width the layer hierarchy remains recognizable, while the click-through SVG preserves full text legibility.

### Next highest-value opportunity

Apply the same product-intelligence visual language to the repository's technical architecture document, keeping deployment details separate from the learner-facing README narrative.
