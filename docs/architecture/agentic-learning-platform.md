...
- 浏览器：草稿、最多 500 条安全学习事件和离线确定性总教练；500 条由最多 200 条纵向里程碑与至少 300 条最近事件组成，事件本身也是断网可恢复 outbox。
- 静态 CDN：React 应用和版本化题库 JSON。
- Fastify 网关：学习 API、Agent 编排、模型代理、运行/提交入口。
- Judge0：隔离执行不可信代码。
- 当前简单部署：单实例、单用户网关文件仓储 `/data/learning.json`；事件正文按与浏览器一致的纵向投影保留 500 条，另保留 5,000 个事件回放收据用于幂等校验和可证明的里程碑恢复。PostgreSQL 不截断原始学习事件，是登录用户的权威历史；投影不生成任何不存在的证据，也不直接修改掌握度。
